"""Azure Speech STT/TTS. SDK 는 블로킹 — 호출측(app/services/speech.py)이 to_thread 로 감싼다.

필요 env: AZURE_SPEECH_KEY, AZURE_SPEECH_REGION(기본 koreacentral),
...
def _resolve_audio_bytes(audio: dict) -> bytes:
    """AudioIn dict(kind=base64|url)에서 오디오 바이트 추출."""
    kind = audio.get("kind")
    if kind == "base64":
        data = audio.get("data")
        if not data:
...
def _to_wav(raw: bytes, mime_type: str | None) -> bytes:
    """WebM/OGG 등 → 16kHz 모노 16bit WAV (pydub + ffmpeg)."""
...
def _recognize_once(audio: dict) -> speechsdk.SpeechRecognitionResult:
    """오디오 dict → WAV 변환 → 1회 인식. STT 실행 로직은 여기 한 곳."""
...
def transcribe_audio_detailed(audio: dict) -> dict:
    """STT 결과를 SSE `stt` 이벤트 계약(dict)으로 반환. 예외도 error dict 로 감싼다."""
...
def synthesize_speech_base64(text: str, voice_name: str | None = None) -> str:
    """text → base64 WAV. TTS 로 읽지 않을 마크다운/이모지는 제거."""
