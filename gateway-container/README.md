...
`-- app/
    ...
    |-- api.py             v1 엔드포인트 전체
    |-- respond_flow.py    상담 한 턴의 전체 흐름 (핵심) + 요청 정리
    |-- context_policy.py  라벨별 응답 정책 (사람 편집)
    |-- crisis.py          위기 메시지/핫라인 (사람 편집)
    |-- prompts.py         시스템 프롬프트/답변 스타일 (사람 편집)
    |-- auth.py            인증 (+ Entra 도입 가이드)
    |-- contracts.py       요청 Pydantic 모델
    |-- events.py          SSE 이벤트 payload
    |-- ranking.py         RAG 재정렬 (+ 튜닝 노브)
    |-- session.py         세션 저장소 (memory/Cosmos) + 턴 빌더
    |-- settings.py        환경변수
    `-- services/          외부 서비스 창구 — 컴포넌트당 파일 하나 (어댑터+클라이언트 통합)
...
3. app/respond_flow.py     상담 한 턴의 전체 흐름 (핵심)
4. app/context_policy.py   라벨별 응답 정책 (사람 편집)
...
라벨별 응답 정책       app/context_policy.py   POLICIES 테이블 (+ 저확신 강등 노브)
위기 메시지/핫라인     app/crisis.py           + 위치 기반 DB 조회 작업 가이드
