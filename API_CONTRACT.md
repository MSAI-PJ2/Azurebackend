...
Azure revision: api-gateway--0000021 (3-2 ??, 3-3 Cosmos ?? ?)
...
```text
memory: ???. Container App revision/replica ?? ? ??? ??? ? ?? ?? ???.
cosmos: Azure Cosmos DB NoSQL ????? ?? turn? ???? ?? ???.
```
...
???? ??? ?: /session_id
?? id: session_id? ??
?? ?? ??: id, session_id, created_at, updated_at, turns[], ttl
SESSION_TTL_SECONDS > 0?? Cosmos ?? ttl ???? ?? ? ??? ???.
```
...
```text
session_id: ??? Gateway? ??? ? ???, ??? ?????? ???? ?? ???? ??.
turn_count: ?? ?? ? user/assistant/system? turn ?.
turns: user ??, assistant ??, crisis/input_required/STT ?? ? ????? ??? ??.
??: Cosmos DB? ?????? ????/???? ?? ??, ?? ??, ?? ?? ??? ?? ?? ???? ???? ??.
```
